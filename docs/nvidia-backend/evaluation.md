...
sweep used the `6f9a0b78` artifact label; the current three-repeat compact
tensor-baseline sweep uses `47d857e1`:
...
- [Current capture](evaluation-current.md) records a compact tensor-baseline
  ...
  SGEMM rows for `16x16x16` and `16x16x64` descriptors with three samples
  per GPU/shape/baseline.
