...
inline bool validate_paged_attention_shape(uint64_t num_heads, uint64_t q_tile, uint64_t block_size, uint64_t head_dim) {
    if (head_dim == 128 && ((q_tile == 16 && block_size == 128) || (q_tile == 64 && block_size == 64))) {
        return true;
    }

    pto2_rt_report_fatal(
        PTO2_ERROR_INVALID_ARGS,
        "paged_attention supports only (q_tile=16, block_size=128, head_dim=128) or "
        "(q_tile=64, block_size=64, head_dim=128); got num_heads=%" PRIu64 ", q_tile=%" PRIu64
        ", block_size=%" PRIu64 ", head_dim=%" PRIu64,
        num_heads, q_tile, block_size, head_dim
    );
    return false;
}
...
__attribute__((visibility("default"))) void aicpu_orchestration_entry(const ChipStorageTaskArgs &orch_args) {
    ...
    if (!validate_paged_attention_shape(num_heads, q_tile, block_size, head_dim)) {
        return;
    }
