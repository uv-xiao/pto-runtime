...
extern "C" int build_graph_aicpu(Runtime* runtime) {
    ...
    int t0 = aicpu_runtime_add_task(runtime, args_t0, 4, 0, CoreType::AIV, runtime->kernel_addrs[0]);
    if (t0 < 0) return -1;
    aicpu_runtime_publish_task(runtime, t0);  // root task can be scheduled immediately

    // In concurrent mode, publish runnable roots as early as possible by leaving them fanin==0.
    // The scheduler will pick up tasks from its ready queues.
    ...
    int t1 = aicpu_runtime_add_task(runtime, args_t1, 4, 1, CoreType::AIV, runtime->kernel_addrs[1]);
    if (t1 < 0) return -1;
    ...
    int t2 = aicpu_runtime_add_task(runtime, args_t2, 4, 1, CoreType::AIV, runtime->kernel_addrs[1]);
    if (t2 < 0) return -1;
    ...
    int t3 = aicpu_runtime_add_task(runtime, args_t3, 4, 2, CoreType::AIV, runtime->kernel_addrs[2]);
    if (t3 < 0) return -1;
